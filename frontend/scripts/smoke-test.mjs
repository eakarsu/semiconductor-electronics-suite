const baseUrl = process.env.SMOKE_BASE_URL || 'http://127.0.0.1:4680';
...
const adminCookie = await login('admin@semiconductor-electronics.local', 'admin123');
const managerCookie = await login('manager@semiconductor-electronics.local', 'manager123');
const analystCookie = await login('analyst@semiconductor-electronics.local', 'analyst123');

await expectJson('/api/dashboard', adminCookie);
await expectJson('/api/entities/lots', analystCookie);
await expectJson('/api/documents', analystCookie);
await expectStatus('/api/documents/upload', analystCookie, 405);

const records = await expectJson('/api/entities/lots', managerCookie);
const rowId = records.rows[0].id;

const approveResponse = await fetch(`${baseUrl}/api/entities/lots/approve`, {
  method: 'POST',
  headers: {
...
const forbiddenApprove = await fetch(`${baseUrl}/api/entities/lots/approve`, {
  method: 'POST',
  headers: {
